...
function getMpvOptions(options, mediaType, mediaSource) {
    ...
    if (options.displaySync) {
        ...
        var displayParams_active = require("electron").screen.getDisplayNearestPoint({
            x: winPosition[0],
            y: winPosition[1],
        });
        ...
        if (
            winBounds.width == displayParams_active.size.width &&
            displayParams_active.size.height == winBounds.height
        ) {
            var rf_rate =
    ...
    if (options.largeCache) {
        ...
        var backBuffer = Math.round(cacheSize * 0.8);
...
function getStatsJson(player) {
    return Promise.all([getMediaStats(player), getVideoStats(player), getAudioStats(player)]).then(function (
        responses
    ) {
        var categories = [];
